...
local cfg = inicfg.load({
    ...
    phone = {
        enabled = true,
        greeting = 'Здравствуйте, я Заместитель Министра здравоохранения. Вы что-то хотели?'
    },
    gos = {
...
if cfg.main.show_heal_debug == nil then cfg.main.show_heal_debug = false end
if cfg.main.hide_smi_ads == nil then cfg.main.hide_smi_ads = false end
if cfg.main.hide_job_chat == nil then cfg.main.hide_job_chat = false end
...
if cfg.phone.greeting == nil or tostring(cfg.phone.greeting) == '' then
    cfg.phone.greeting = 'Здравствуйте, я Заместитель Министра здравоохранения. Вы что-то хотели?'
end
MSH_PHONE_GREET_ENABLED = imgui.new.bool(cfg.phone.enabled == true)
MSH_PHONE_GREET_BUFFER = imgui.new.char[256](cfg.phone.greeting or 'Здравствуйте, я Заместитель Министра здравоохранения. Вы что-то хотели?')
...
if cfg.prices.ambulance == nil then cfg.prices.ambulance = 800 end
...
cfg.rptexts = cfg.rptexts or {
    mask = '/me аккуратно поправил медицинскую маску на лице.',
    ...
    c60 = '/me проверил служебный журнал.'
}
...
function MSHelper_DrawThemeSettings()
    ...
    imgui.TextWrapped('Выберите оформление окна. Тема сохраняется в ms_helper.ini и остается после перезапуска.')
    local current = MSHelper_GetThemeKey()
    for i, key in ipairs(MSHelper_ThemeOrder) do
        if i > 1 and ((i - 1) % 3) ~= 0 then imgui.SameLine() end
        local label = MSHelper_ThemeLabels[key] or key
        if imgui.Selectable(label .. '##ms_theme_' .. key, current == key, 0, imgui.ImVec2(150, 28)) then
...
function _G.MSHelper_DrawOkReportBlock()
    ...
    imgui.TextWrapped('Команда: /ok id. В /r уйдет: Ник Фамилия, Ваш доклад принят. Если ID не указан, скрипт не отправляет 0.')
end
...
local function run_invite_sequence_inline(pid, nick)
    pid = tonumber(pid) or -1
...
function sampev.onServerMessage(color, text)
    if text then
        ...
        -- Телефонное приветствие теперь срабатывает только после системного ответа сервера,
        -- что трубка действительно взята/звонок принят. Один ввод /p сам по себе текст не отправляет.
        if (MSH_PHONE_GREET_WAIT_UNTIL or 0) > os.clock() then
            if MSH_PHONE_GREET_IS_ACCEPTED(raw_text, low_text) then
                MSH_PHONE_GREET_SEND_ACCEPTED()
            end
        elseif (MSH_PHONE_GREET_WAIT_UNTIL or 0) ~= 0 then
            MSH_PHONE_GREET_WAIT_UNTIL = 0
        end

        -- Если сервер ответил антифлудом на /invite, не продолжаем цепочку и не шлем /r.
        ...
        if cfg.main.hide_job_chat and MSHelper_IsJobChatMessage(raw_text) then
            return false
        end
...
local function get_action_key_from_command(command)
    ...
    if cmd == '/c 60' or cmd == '/c 060' or cmd:match('^/c%s+0?60%s*$') then return 'c60' end
...
send_ambulance_heal_sequence = function(pid, amount)
    ...
    if not amount then
        amount = (price.ambulance and tonumber(price.ambulance[0])) or tonumber(cfg.prices.ambulance) or 800
    end
...
local function toggle(label, key)
    local v = imgui.new.bool(cfg.main[key])
    if imgui.Checkbox(label, v) then cfg.main[key] = v[0]; save() end
end
...
action_rp = function(key, cmd)
    ...
    lua_thread.create(function()
        if action_enabled[key] and action_enabled[key][0] then
...
local function draw_action_rp_editor()
    ...
    imgui.TextWrapped('Нажми на кнопку команды выше, чтобы выбрать её. Галочка справа включает/выключает RP перед командой.')
    ...
    imgui.SameLine()
    if imgui.Button('Очистить RP', imgui.ImVec2(120, 28)) then
        ffi.copy(action_rp_buffers[selected_action_edit], '')
        cfg.rptexts[selected_action_edit] = ''
        save()
    end
end
...
local function split_lines(text)
    ...
    for part in string.gmatch(text, '([^|]+)') do
        ...
        if #part > 0 then
            -- Разделитель/перенос строк биндера отключен: отправляем как есть.
...
local function draw_binder_tab()
    ...
    imgui.Separator()
    imgui.TextWrapped('Пример: в текст можно вписать /me достал аптечку|/do Аптечка в руках.|Здравствуйте.')
...
function MS_ShporaSetServerMode(mode, key)
    ...
    MS_ShporaLoadCache()
    MS_SHPORA_STATUS = 'Выбран сервер шпаргалок: ' .. MS_ShporaServerLabel(MS_ShporaCurrentServerKey()) .. ' (' .. MS_ShporaModeText() .. ').'
end
...
function MS_ECPForceServerPrices()
    cfg.prices = cfg.prices or {}
    MS_ECP_FORCE_TMP_KEY = 'green'
    if type(MS_ShporaCurrentServerKey) == 'function' then
        MS_ECP_FORCE_TMP_OK, MS_ECP_FORCE_TMP_RESULT = pcall(MS_ShporaCurrentServerKey)
        if MS_ECP_FORCE_TMP_OK and MS_ECP_FORCE_TMP_RESULT then
            MS_ECP_FORCE_TMP_KEY = tostring(MS_ECP_FORCE_TMP_RESULT):lower()
        end
    end

    if MS_ECP_FORCE_TMP_KEY == 'blue' then
        cfg.prices.mvd = 1
        cfg.prices.pravo = 3000
        cfg.prices.mz = 3000
        cfg.prices.mo = 1
        cfg.prices.smi = 3000
        cfg.prices.civil = 800
        cfg.prices.heal = 800
        cfg.prices.ambulance = 300
        cfg.prices.private = 3000
        cfg.prices.lkn = 3000
        cfg.prices.yakuza = 3000
        cfg.prices.rm = 3000
        cfg.prices.gangs = 2500
        cfg.prices.surgery = 50000
        cfg.prices.gender = 5000
    elseif MS_ECP_FORCE_TMP_KEY == 'chocolate' then
        cfg.prices.mvd = 1000
        cfg.prices.pravo = 1000
        cfg.prices.mz = 500
        cfg.prices.mo = 1000
        cfg.prices.smi = 1500
        cfg.prices.civil = 500
        cfg.prices.heal = 500
        cfg.prices.ambulance = 1000
        cfg.prices.private = 2000
        cfg.prices.lkn = 2000
        cfg.prices.yakuza = 2000
        cfg.prices.rm = 2000
        cfg.prices.gangs = 2000
        cfg.prices.surgery = 5000
        cfg.prices.gender = 10000
    elseif MS_ECP_FORCE_TMP_KEY == 'lime' then
        cfg.prices.mvd = 250
        cfg.prices.pravo = 250
        cfg.prices.mz = 250
        cfg.prices.mo = 250
        cfg.prices.smi = 250
        cfg.prices.civil = 250
        cfg.prices.heal = 250
        cfg.prices.ambulance = 1000
        cfg.prices.private = 250
        cfg.prices.lkn = 250
        cfg.prices.yakuza = 250
        cfg.prices.rm = 250
        cfg.prices.gangs = 250
        cfg.prices.surgery = 5000
        cfg.prices.gender = 10000
    elseif MS_ECP_FORCE_TMP_KEY == 'green' then
        cfg.prices.mvd = 1000
        cfg.prices.pravo = 1000
        cfg.prices.mz = 1000
        cfg.prices.mo = 1000
        cfg.prices.smi = 1000
        cfg.prices.civil = 1000
        cfg.prices.heal = 1000
        cfg.prices.ambulance = 1000
        cfg.prices.private = 2000
        cfg.prices.lkn = 2000
        cfg.prices.yakuza = 2000
        cfg.prices.rm = 2000
        cfg.prices.gangs = 2000
        cfg.prices.surgery = 2000
        cfg.prices.gender = 50000
    elseif MS_ECP_FORCE_TMP_KEY == 'red' then
        cfg.prices.mvd = 250
        cfg.prices.pravo = 250
        cfg.prices.mz = 0
        cfg.prices.mo = 250
    ...
    if price then
        if price.mvd then price.mvd[0] = cfg.prices.mvd end
        if price.pravo then price.pravo[0] = cfg.prices.pravo end
        if price.mz then price.mz[0] = cfg.prices.mz end
        if price.mo then price.mo[0] = cfg.prices.mo end
        if price.smi then price.smi[0] = cfg.prices.smi end
        if price.civil then price.civil[0] = cfg.prices.civil end
        if price.heal then price.heal[0] = cfg.prices.heal end
        if price.ambulance then price.ambulance[0] = cfg.prices.ambulance end
        if price.lkn then price.lkn[0] = cfg.prices.private end
        if price.yakuza then price.yakuza[0] = cfg.prices.private end
        if price.rm then price.rm[0] = cfg.prices.private end
        if price.gangs then price.gangs[0] = cfg.prices.gangs end
    end
end

function MS_ShporaRefreshECPDisplay()
    cfg.prices = cfg.prices or {}
    if type(MS_ECPForceServerPrices) == 'function' then MS_ECPForceServerPrices() end
    ...
    MS_ECP_DISPLAY.mo = tonumber(cfg.prices.mo) or 1
    MS_ECP_DISPLAY.smi = tonumber(cfg.prices.smi) or 1
    MS_ECP_DISPLAY.civil = tonumber(cfg.prices.civil) or 800
    MS_ECP_DISPLAY.ambulance = tonumber(cfg.prices.ambulance) or 800
    ...
    MS_ECP_DISPLAY.heal = tonumber(cfg.prices.heal) or 300
end
...
function MS_ShporaSetPriceKey(key, num)
    ...
    elseif key == 'lkn' or key == 'yakuza' or key == 'rm' or key == 'mvd' or key == 'pravo' or key == 'mz' or key == 'mo' or key == 'smi' or key == 'civil' or key == 'gangs' or key == 'heal' or key == 'ambulance' then
        cfg.prices[key] = num
        ...
        if MS_SHPORA_TMP_KEY and MS_SHPORA_TMP_VALUE then
            ...
            if MS_SHPORA_TMP_NUM and not MS_SHPORA_TMP_SKIP then
                ...
                if MS_SHPORA_TMP_LOW:find('стандартное лечение', 1, true) or MS_SHPORA_TMP_LOW:find('все жители', 1, true) or MS_SHPORA_TMP_LOW:find('всех граждан', 1, true) then
                    ...
                    if MS_ShporaSetPriceKey('heal', MS_SHPORA_TMP_NUM) then count = count + 1 end
                end
...
function MS_ShporaDrawServerSelector()
    ...
    imgui.TextWrapped('Авто выбирает ЕЦП/Устав по текущему серверу. Если авто не сработало — выберите сервер вручную.')
...
imgui.OnFrame(function() return main_window[0] end, function()
    ...
    if imgui.Button('Закрыть', imgui.ImVec2(135, 32)) then main_window[0] = false end
    imgui.EndChild(); imgui.SameLine()
    imgui.BeginChild('content', imgui.ImVec2(0, 0), true)

    if active_tab == 1 then
        imgui.TextColored(MSHelper_AccentColor(), 'Основное')
        MSHelper_DrawThemeSettings()
        toggle('Скрывать объявления СМИ', 'hide_smi_ads')
        toggle('Скрывать рабочий чат /j и /jn', 'hide_job_chat')
        toggle('Показывать отладку лечения', 'show_heal_debug')
        _G.MSHelper_DrawRadioTagSettings()

        imgui.Separator()
        imgui.TextColored(MSHelper_AccentColor(), 'Телефон /p')
        imgui.TextWrapped('Сработает только после системного сообщения сервера, что трубка действительно взята через /p.')
        if imgui.Checkbox('Авто-приветствие после /p##phone_greet_enabled', MSH_PHONE_GREET_ENABLED) then
            save()
        end
        imgui.PushItemWidth(-1)
        if imgui.InputText('Приветствие для /p##phone_greet_text', MSH_PHONE_GREET_BUFFER, 256) then
            save()
        end
        imgui.PopItemWidth()
...
imgui.OnFrame(function() return patient_window[0] end, function()
    ...
    if imgui.Button('Выдать мед.карту', imgui.ImVec2(-1, 27)) then
        patient_window[0] = false
        sampSendChat('/givemed ' .. selected_patient)
    end
...
imgui.OnFrame(function() return interview_window[0] end, function()
    ...
    if interview_step >= 5 then
        if imgui.Button('5. Подходит — отправить приглашение /invite', imgui.ImVec2(-1, 34)) then
            send_interview_step(5)
        end
    else
        imgui.TextDisabled('5. Подходит — отправить приглашение /invite')
    end
...
local function is_any_ms_window_open()
...
local function close_all_ms_windows()
...
local function gta_pause_menu_active()
...
local drone_thread_running = false
local drone_x, drone_y, drone_z = 0.0, 0.0, 0.0
local drone_yaw, drone_pitch = 0.0, 0.0

-- Сделал скорость намного медленнее. Меняется клавишами - и +.
local drone_speed = 0.06
local drone_speed_min = 0.01
local drone_speed_max = 0.75
local drone_speed_step = 0.01
local drone_mouse_sens = 0.0025
...
local function drone_speed_text()
    return string.format('%.2f', drone_speed)
end

local function drone_change_speed(delta)
...
local function drone_lock_player(state)
...
local function drone_restore_camera()
...
local function drone_apply_camera()
    local dist = 8.0
...
local function drone_get_screen_center()
    ...
    lua_thread.create(function()
        ...
        while drone_enabled do
            ...
            if ffi.C.GetCursorPos(pt) ~= 0 then
                ...
                -- Исправление осей HorAssist-style:
...
function MSHelper_CmdJobChat()
    cfg.main.hide_job_chat = not cfg.main.hide_job_chat
    save()
    helper_msg('Скрытие рабочего чата /j и /jn: ' .. (cfg.main.hide_job_chat and 'включено' or 'выключено') .. '.')
end
...
function MSHelper_CmdRp(arg)
    ...
    if not key or not rp or not action_rp_buffers[key] then
        helper_msg('Используйте: /msrp key текст. key: mask find lock healme changeskin c60')
        return
    end
...
function MSHelper_RegisterCommands()
    ...
    sampRegisterChatCommand('ms', MSHelper_CmdOpenMain)
    sampRegisterChatCommand('mgos', MSHelper_CmdToggleGos)
    sampRegisterChatCommand('medinfo', MS_SkladShowInfo)
    sampRegisterChatCommand('medrn', MS_SkladSendRadio)
    sampRegisterChatCommand('shupdate', MS_ShporaUpdateAll)
    sampRegisterChatCommand('shserver', MS_ShporaCmdServer)
    sampRegisterChatCommand('msname', MSHelper_CmdName)
    sampRegisterChatCommand('mwhere', MSHelper_CmdWhere)
    sampRegisterChatCommand('ok', MSHelper_CmdOk)
    sampRegisterChatCommand('sobes', MSHelper_CmdSobes)
    sampRegisterChatCommand('drone', MSHelper_CmdDrone)
    sampRegisterChatCommand('rec', MSHelper_CmdReconnect)
end
...
-- =========================================================
-- MS Helper addon: home marker v13 safe blip MSHelper DB
-- /mshome ID       - поставить GPS/блип метку на дом по ID
-- /mshome ID дом   - тоже работает
-- /mshome off      - убрать метку
-- /mshome update   - перечитать house.ini из config
-- База домов: MSHelper house.ini. Если положить новый house.ini в
-- moonloader\config\house.ini, скрипт будет брать его вместо встроенного.
-- =========================================================
...
local function __ms_home_text(s)
...
local __ms_home = {
    ...
    radius = 3.0,
    -- v22: waypoint + visible dark home marker + server ID auto-offset fix.
    ...
    reapply_ms = 3000,
    max_server_id = 1323,
    -- Старые базы домов HorAssist/MSHelper часто начинаются с [0],
    -- а на сервере игрок вводит ID с 1. Поэтому по умолчанию
    -- /mshome 15 будет брать координаты секции [14], если база начинается с [0].
...
local function __ms_home_trim(s)
    return (tostring(s or ''):gsub('^%s+', ''):gsub('%s+$', ''))
end

local function __ms_home_file_exists(path)
...
local function __ms_home_read_file(path)
    local ok, f = pcall(io.open, path, 'rb')
...
local function __ms_home_write_file(path, data)
...
local function __ms_home_prepare_dirs()
...
local function __ms_home_remember(id, x, y, z)
    ...
    __ms_home.count = __ms_home.count + 1
...
local function __ms_home_parse_db(data)
    local houses = {}
    __ms_home.houses = houses
    ...
    local function flush()
        if current_id ~= nil and x ~= nil and y ~= nil and z ~= nil then
            __ms_home_remember(current_id, x, y, z)
        end
    end
        ...
        if line ~= '' and not line:match('^;') and not line:match('^#') then
            ...
            if sid then
                ...
                if key and value then
                    ...
                    if #nums >= 4 then
                        __ms_home_remember(nums[1], nums[2], nums[3], nums[4])
                    end
                end
            end
        end
    ...
    flush()
    return __ms_home.count > 0
end

local function __ms_home_load_db(silent)
    __ms_home_prepare_dirs()

    local data = nil
    if __ms_home_file_exists(__ms_home.user_path) then
        data = __ms_home_read_file(__ms_home.user_path)
        __ms_home.db_source = __ms_home.user_path
    elseif __ms_home_file_exists(__ms_home.cache_path) then
        data = __ms_home_read_file(__ms_home.cache_path)
        __ms_home.db_source = __ms_home.cache_path
    elseif __ms_home_file_exists(__ms_home.local_path) then
        data = __ms_home_read_file(__ms_home.local_path)
        __ms_home.db_source = __ms_home.local_path
    else
        data = __ms_home_embedded_db
        __ms_home.db_source = 'embedded MSHelper house.ini'
        __ms_home_write_file(__ms_home.cache_path, data)
    end
    ...
    local ok = __ms_home_parse_db(data)
    if not ok then
        data = __ms_home_embedded_db
        __ms_home.db_source = 'embedded MSHelper house.ini'
        __ms_home_write_file(__ms_home.cache_path, data)
        ok = __ms_home_parse_db(data)
    end

    if not silent then
        if ok then
            __ms_home_msg(string.format('[MS Home] База домов загружена: %d домов, ID %s-%s.', __ms_home.count, tostring(__ms_home.min_id), tostring(__ms_home.max_id)), 0x66CCFFFF)
            if (__ms_home.max_id or 0) < __ms_home.max_server_id then
                __ms_home_msg(string.format('[MS Home] В этой базе нет домов выше ID %d. Для 1323 нужен свежий house.ini.', __ms_home.max_id or 0), 0xFFFFC040)
            end
        else
...
local function __ms_home_remove_one_blip(blip)
...
local function __ms_home_remove_marker()
...
local function __ms_home_try_call(name, ...)
...
local function __ms_home_setup_blip(blip, color, scale)
    if blip == nil then return end
...
local function __ms_home_add_coord_blip(x, y, z, color, scale)
...
local function __ms_home_try_sprite_blip(x, y, z)
...
local function __ms_home_try_waypoint(x, y, z)
...
local function __ms_home_set_marker(x, y, z)
...
local function __ms_home_disable(silent)
...
local function __ms_home_get_house_for_server_id(id, raw_mode)
    id = tonumber(id)
    if id == nil then return nil, nil, nil end
    id = math.floor(id)

    -- raw_mode нужен на случай, если у игрока база уже полностью совпадает с ID сервера.
    -- Использование: /mshome 15 raw
    if raw_mode then
        return __ms_home.houses[id], id, false
    end

    -- Важно: большинство старых баз домов начинается с [0], а серверные ID обычно с 1.
    -- Из-за этого старая версия ставила метку на соседний дом.
    -- Если база начинается с 0, автоматически используем ID-1.
    if __ms_home.prefer_zero_based ~= false and (__ms_home.min_id or 0) == 0 and id >= 1 then
        local shifted_id = id - 1
        if __ms_home.houses[shifted_id] then
            return __ms_home.houses[shifted_id], shifted_id, true
        end
    end
...
local function __ms_home_mark_by_id(id, raw_mode)
    id = tonumber(id)
    ...
    local house, db_id, shifted = __ms_home_get_house_for_server_id(id, raw_mode)
    if not house then
        __ms_home_msg(string.format('[MS Home] Дом ID %d не найден в базе.', id), 0xFFFF4040)
        if (__ms_home.max_id or 0) < __ms_home.max_server_id and id > ((__ms_home.max_id or 0) + 1) then
            __ms_home_msg(string.format('[MS Home] Сейчас в базе MSHelper только до ID %d. Нужен свежий house.ini.', (__ms_home.max_id or 0) + 1), 0xFFFFC040)
        end
        return false
    end
    ...
    if __ms_home_set_marker(house.x, house.y, house.z) then
        ...
        if shifted then
            __ms_home_msg(string.format('[MS Home] Метка на дом ID %d поставлена. База ID: %d. Режим: %s.', id, db_id, tostring(__ms_home.marker_mode or 'unknown')), 0x66CCFFFF)
        else
            __ms_home_msg(string.format('[MS Home] Метка на дом ID %d поставлена. Режим: %s.', id, tostring(__ms_home.marker_mode or 'unknown')), 0x66CCFFFF)
        end
        return true
    end
    return false
...
local function __ms_home_usage()
    __ms_home_msg('[MS Home] /mshome ID - метка на дом по ID.', 0x66CCFFFF)
    ...
    -- Скрытая команда: /mshome ID raw (для отладки базы, в подсказке не показываем)
end

local function __ms_home_command(arg)
...
function MSH_UpdDir()
    ...
    if type(thisScript) == 'function' then
        ...
        if ok and script and script.path then
            local dir = tostring(script.path):match('^(.*)[/\\][^/\\]+$')
...
function MSH_UpdCurrentPath()
    ...
    return MSH_UpdDir() .. '\\MSHelper.lua'
end

function MSH_UpdJoin(name)
    return MSH_UpdDir() .. '\\' .. tostring(name or '')
end

MSH_UPDATER_VERSION_FILE = MSH_UpdJoin('MSHelper_update_version.txt')
MSH_UPDATER_DOWNLOAD_FILE = MSH_UpdJoin('MSHelper_update.lua')
MSH_UPDATER_TARGET_FILE = MSH_UpdCurrentPath()
MSH_UPDATER_BACKUP_FILE = MSH_UPDATER_TARGET_FILE .. '.bak'
MSH_UPDATER_BUSY = false
MSH_UPDATER_LAST_CHECK = 0

function MSH_UpdText(s)
    if u8 and u8.decode then
        local ok, r = pcall(function() return u8:decode(tostring(s or '')) end)
        if ok and r then return r end
    end
    return tostring(s or '')
end

function MSH_UpdMsg(s, color)
    if type(sampAddChatMessage) == 'function' then
        pcall(sampAddChatMessage, MSH_UpdText('{159A9C}[MS Update] {FFFFFF}' .. tostring(s or '')), color or -1)
    end
end

function MSH_UpdRead(path)
    local ok, f = pcall(io.open, path, 'rb')
    if not ok or not f then return nil end
...
function MSH_UpdWrite(path, data)
    local ok, f = pcall(io.open, path, 'wb')
    if not ok or not f then return false end
    local ok2 = pcall(function() f:write(tostring(data or '')) end)
    pcall(function() f:close() end)
    return ok2 == true
end
...
function MSH_UpdDownload(url, path, callback)
    if type(downloadUrlToFile) ~= 'function' then
        MSH_UpdMsg('В этой сборке нет downloadUrlToFile, автообновление недоступно.', 0xFFFF4040)
    ...
    local ok = pcall(function()
        downloadUrlToFile(url, path, function(id, status)
            ...
            if download_status and status == download_status.STATUS_ENDDOWNLOADDATA then finished = true end
            if tostring(status) == '6' or tostring(status):lower():find('end') then finished = true end
            if finished and callback then callback(true) end
        end)
    end)
    if not ok then
        MSH_UpdMsg('Не удалось начать скачивание обновления.', 0xFFFF4040)
        if callback then callback(false) end
        return false
    end
    return true
...
function MSH_CheckUpdate(silent, auto_apply)
    if MSH_UPDATER_BUSY then return end
    MSH_UPDATER_BUSY = true
    if not silent then MSH_UpdMsg('Проверяю обновление...') end
    MSH_UpdDownload(MSH_UPDATER_VERSION_URL, MSH_UPDATER_VERSION_FILE, function(ok)
        MSH_UPDATER_BUSY = false
        ...
        MSH_UpdDownload(info.url, MSH_UPDATER_DOWNLOAD_FILE, function(ok2)
            MSH_UPDATER_BUSY = false
            if ok2 and MSH_UpdRead(MSH_UPDATER_DOWNLOAD_FILE) then
                if auto_apply then
                    MSH_ApplyUpdate('auto')
                else
                    MSH_UpdMsg('Обновление скачано. Напиши /msapplyupdate, чтобы заменить старый файл новым.')
                end
            else
                MSH_UpdMsg('Не удалось скачать файл обновления.', 0xFFFF4040)
            end
        end)
    end)
end

function MSH_ApplyUpdate(arg)
    local data = MSH_UpdRead(MSH_UPDATER_DOWNLOAD_FILE)
    if not data or #data < 100 then
        MSH_UpdMsg('Файл обновления не найден. Сначала напиши /msupdate.', 0xFFFFC040)
        return
    end

    -- Простая защита от случайного скачивания не того файла.
    if not tostring(data):find("script_name", 1, true) and not tostring(data):find("MS Helper", 1, true) then
        MSH_UpdMsg('Файл обновления не похож на MSHelper.lua. Замена отменена.', 0xFFFF4040)
        return
    end

    local target = MSH_UPDATER_TARGET_FILE or MSH_UpdCurrentPath()
    local backup = tostring(target) .. '.bak'

    -- Если скрипт запущен как .luac, обновление всё равно ставим как MSHelper.lua,
    -- потому что скачивается обычный исходник .lua.
    local running = MSH_UpdCurrentPath()
    if tostring(target):lower():match('%.luac$') then
        target = MSH_UpdJoin('MSHelper.lua')
        backup = target .. '.bak'
    end

    local old = MSH_UpdRead(target)
    if old and #old > 0 then
        MSH_UpdWrite(backup, old)
    end

    -- Сначала удаляем старый файл, потом записываем новый.
    pcall(os.remove, target)

    if MSH_UpdWrite(target, data) then
        pcall(os.remove, MSH_UPDATER_DOWNLOAD_FILE)
        -- Если был запущен старый .luac, убираем его, чтобы после перезапуска не было конфликта.
        if tostring(running):lower():match('%.luac$') then
            pcall(os.remove, running)
        end
        MSH_UpdMsg('Обновление применено: старый файл удалён, новый установлен. Перезагрузи MoonLoader или игру.')
        MSH_UpdMsg('Резервная копия старого файла: ' .. tostring(backup))
    else
        if old and #old > 0 then
            MSH_UpdWrite(target, old)
        end
        MSH_UpdMsg('Не удалось заменить файл. Старый файл восстановлен, если был backup.', 0xFFFF4040)
    end
end
...
lua_thread.create(function()
    repeat wait(0) until type(isSampAvailable) == 'function' and isSampAvailable()
    if type(sampRegisterChatCommand) == 'function' then
        sampRegisterChatCommand('msupdate', function(arg)
            arg = tostring(arg or ''):lower():gsub('^%s+', ''):gsub('%s+$', '')
            if arg == 'install' or arg == 'apply' or arg == 'обновить' or arg == 'ставить' then
                MSH_CheckUpdate(false, true)
            else
                MSH_CheckUpdate(false, false)
            end
        end)
        sampRegisterChatCommand('msapplyupdate', MSH_ApplyUpdate)
    end
    wait(15000)
    MSH_CheckUpdate(true)
    while true do
        wait(3600000)
        MSH_CheckUpdate(true)
    end
