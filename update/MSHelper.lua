...
-- =========================================================
MSH_UPDATER_VERSION = MSH_UPDATER_VERSION or '1.0.3'
...
function MSHelper_UpdateNoCacheUrl(url)
    url = tostring(url or '')
    if url == '' then return url end
    local sep = url:find('?', 1, true) and '&' or '?'
    return url .. sep .. 'msht=' .. tostring(os.time()) .. tostring(math.random(1000,9999))
end
...
function MSHelper_UpdateHandleJson(raw, json_path, upd_path, force)
    ...
    downloadUrlToFile(MSHelper_UpdateNoCacheUrl(updateurl), upd_path, function(id2, status2)
...
function MSHelper_UpdateRun(arg)
    ...
    lua_thread.create(function()
        ...
        downloadUrlToFile(MSHelper_UpdateNoCacheUrl(MSH_UPDATER_JSON_URL), json_path, function(id, status)
