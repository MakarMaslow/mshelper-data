...
MSHELPER_VERSION = MSHELPER_VERSION or '1.0.1'
MSHELPER_BUILD = MSHELPER_BUILD or 'updater-test'
...
local function draw_commands_tab()
    ...
    cmd('/msupdate', 'Проверить новую версию MS Helper на GitHub.')
    cmd('/msupdate force', 'Скачать файл обновления принудительно для теста.')
...
function main()
    ...
    helper_msg('Загружен /ms Меню BETA 1.0. Версия: ' .. tostring(MSHELPER_VERSION or '1.0.1'))
    while true do
        wait(0)
        MSHelper_MainTick()
    end
...
-- =========================================================
-- MS Helper | безопасное обновление через GitHub
-- Команды: /msupdate, /msupdate force, /msupdate status
-- Файлы GitHub: update/version.txt + update/MSHelper.lua или update/MSHelper.luac
-- =========================================================
MS_UPDATE_BUSY = MS_UPDATE_BUSY or false
MS_UPDATE_VERSION_URL = MS_UPDATE_VERSION_URL or 'https://raw.githubusercontent.com/MakarMaslow/mshelper-data/main/update/version.txt'
MS_UPDATE_DEFAULT_FILE = MS_UPDATE_DEFAULT_FILE or 'MSHelper.lua'
MS_UPDATE_DEFAULT_RAW_BASE = MS_UPDATE_DEFAULT_RAW_BASE or 'https://raw.githubusercontent.com/MakarMaslow/mshelper-data/main/update/'

function MS_UpdateMsg(text)
    if helper_msg then helper_msg(tostring(text or ''))
    elseif type(sampAddChatMessage) == 'function' then sampAddChatMessage('{159A9C}[MS Helper] {FFFFFF}' .. tostring(text or ''), -1) end
end

function MS_UpdateWorkingDir()
    if type(getWorkingDirectory) == 'function' then return tostring(getWorkingDirectory()) end
    return '.'
end

function MS_UpdateNormPath(path)
    return tostring(path or ''):gsub('/', '\\'):lower()
end

function MS_UpdateBaseDir()
    local base = MS_UpdateWorkingDir() .. '\\MSHelper'
    if MS_ShporaMkdir then pcall(MS_ShporaMkdir, base)
    elseif type(createDirectory) == 'function' then pcall(createDirectory, base) end
    return base
end

function MS_UpdatePath(name)
    return MS_UpdateBaseDir() .. '\\' .. tostring(name or '')
end

function MS_UpdateReadFile(path)
    local f = io.open(tostring(path or ''), 'rb')
    if not f then return nil end
    local data = f:read('*a')
    f:close()
...
function MS_UpdateWriteFile(path, data)
    local f = io.open(tostring(path or ''), 'wb')
    if not f then return false end
    f:write(tostring(data or ''))
    f:close()
...
function MS_UpdateFileSize(path)
    local f = io.open(tostring(path or ''), 'rb')
...
function MS_UpdateFileExists(path)
    return MS_UpdateFileSize(path) > 0
end

function MS_UpdateParseKV(data)
    local t = {}
    data = tostring(data or ''):gsub('^\239\187\191', '')
...
function MS_UpdateVersionCompare(a, b)
    local function parts(s)
        local r = {}
        for n in tostring(s or ''):gmatch('%d+') do r[#r + 1] = tonumber(n) or 0 end
        return r
    end
    local aa, bb = parts(a), parts(b)
    local maxn = math.max(#aa, #bb)
    for i = 1, maxn do
...
function MS_UpdateSafeFileName(name)
    name = tostring(name or ''):gsub('^%s+', ''):gsub('%s+$', '')
    name = name:gsub('[\\/:%*%?"<>|]', '')
    if name == '' then name = MS_UPDATE_DEFAULT_FILE end
    if not name:lower():match('^mshelper%.lua[c]?$') then
        name = MS_UPDATE_DEFAULT_FILE
    end
    return name
end

function MS_UpdateCurrentScriptPath()
    local ok, sc = pcall(thisScript)
    if ok and sc and sc.path and tostring(sc.path) ~= '' then return tostring(sc.path) end
    return MS_UpdateWorkingDir() .. '\\MSHelper.lua'
end

function MS_UpdateDownload(url, path, min_size)
    url = tostring(url or '')
    path = tostring(path or '')
    min_size = tonumber(min_size) or 20
    if url == '' or path == '' or type(downloadUrlToFile) ~= 'function' then return false, -1 end
    ...
    for i = 1, 160 do
        wait(250)
        local size = MS_UpdateFileSize(path)
        if size >= min_size then
            ...
            if stable_ticks >= 3 then return true, last_status end
        end
    end
    return MS_UpdateFileSize(path) >= min_size, last_status
end

function MS_UpdateWritePending(new_path, target_path, old_path, version)
    local data = table.concat({
        'version=' .. tostring(version or ''),
        'new_path=' .. tostring(new_path or ''),
        'target_path=' .. tostring(target_path or ''),
        'old_path=' .. tostring(old_path or ''),
        'target_backup=' .. tostring(target_path or '') .. '.backup',
        'old_backup=' .. tostring(old_path or '') .. '.backup',
        'created_at=' .. os.date('%d.%m.%Y %H:%M:%S')
    }, '\n')
    return MS_UpdateWriteFile(MS_UpdatePath('update_pending.ini'), data)
end

function MS_UpdateInstallNow(new_path, target_path, old_path, version)
    new_path = tostring(new_path or '')
    target_path = tostring(target_path or '')
    old_path = tostring(old_path or '')
    if not MS_UpdateFileExists(new_path) then return false, 'новый файл не найден' end
    if target_path == '' then return false, 'не указан путь установки' end

    local target_backup = target_path .. '.backup'
    local old_backup = old_path .. '.backup'
    local same_target_old = (MS_UpdateNormPath(target_path) == MS_UpdateNormPath(old_path))

    os.remove(target_backup)
    if not same_target_old then os.remove(old_backup) end

    if same_target_old then
        if MS_UpdateFileExists(target_path) then
            local ok_backup = os.rename(target_path, target_backup)
            if not ok_backup then return false, 'не удалось убрать старый файл' end
        end
        local ok_new = os.rename(new_path, target_path)
        if not ok_new then
            if MS_UpdateFileExists(target_backup) then os.rename(target_backup, target_path) end
            return false, 'не удалось поставить новый файл'
        end
    else
        if MS_UpdateFileExists(target_path) then
            local ok_target_backup = os.rename(target_path, target_backup)
            if not ok_target_backup then return false, 'не удалось убрать старый целевой файл' end
        end
        if old_path ~= '' and MS_UpdateFileExists(old_path) then
            local ok_old_backup = os.rename(old_path, old_backup)
            if not ok_old_backup then
                if MS_UpdateFileExists(target_backup) then os.rename(target_backup, target_path) end
                return false, 'не удалось убрать старый MSHelper'
            end
        end
        local ok_new = os.rename(new_path, target_path)
        if not ok_new then
            if MS_UpdateFileExists(old_backup) then os.rename(old_backup, old_path) end
            if MS_UpdateFileExists(target_backup) then os.rename(target_backup, target_path) end
            return false, 'не удалось поставить новый файл'
        end
    end

    MS_UpdateWriteFile(MS_UpdatePath('update_installed.txt'), 'version=' .. tostring(version or '') .. '\ninstalled_at=' .. os.date('%d.%m.%Y %H:%M:%S') .. '\n')
    return true, 'ok'
end

function MS_UpdateCommand(arg)
    arg = tostring(arg or ''):lower():gsub('^%s+', ''):gsub('%s+$', '')
    if MS_UPDATE_BUSY then MS_UpdateMsg('Обновление уже выполняется.'); return end
    if arg == 'status' then
        MS_UpdateMsg('Текущая версия: ' .. tostring(MSHELPER_VERSION or 'unknown') .. '. Файл проверки: update/version.txt')
        return
    end
    local force = (arg == 'force' or arg == 'test' or arg == 'тест')

    MS_UPDATE_BUSY = true
    lua_thread.create(function()
        local ok_thread, err_thread = pcall(function()
            MS_UpdateMsg('Проверяю обновление MS Helper...')
            local version_path = MS_UpdatePath('version_check.txt')
            local version_url = MS_UPDATE_VERSION_URL .. '?nocache=' .. tostring(os.time()) .. tostring(math.random(10000, 99999))
            local ok_version, status_version = MS_UpdateDownload(version_url, version_path, 10)
            if not ok_version then
                MS_UpdateMsg('Не удалось скачать update/version.txt с GitHub. Код: ' .. tostring(status_version))
                return
            end

            local kv = MS_UpdateParseKV(MS_UpdateReadFile(version_path) or '')
            local remote_version = tostring(kv.version or '')
            if remote_version == '' then
                MS_UpdateMsg('В update/version.txt нет строки version=')
                return
            end

            local current_version = tostring(MSHELPER_VERSION or '0.0.0')
            if not force and MS_UpdateVersionCompare(remote_version, current_version) <= 0 then
                MS_UpdateMsg('Обновлений нет. Текущая версия: ' .. current_version .. ', на GitHub: ' .. remote_version)
                return
            end

            local file_name = MS_UpdateSafeFileName(kv.file or kv.target or MS_UPDATE_DEFAULT_FILE)
            local file_url = tostring(kv.url or '')
            if file_url == '' then file_url = MS_UPDATE_DEFAULT_RAW_BASE .. file_name end
            local min_size = tonumber(kv.min_size or kv.minsize or 100000) or 100000
            if min_size < 10000 then min_size = 10000 end

            local new_path = MS_UpdatePath('MSHelper.new')
            local old_path = MS_UpdateCurrentScriptPath()
            local target_path = MS_UpdateWorkingDir() .. '\\' .. file_name

            MS_UpdateMsg('Найдена версия ' .. remote_version .. '. Скачиваю ' .. file_name .. '...')
            local ok_file, status_file = MS_UpdateDownload(file_url .. '?nocache=' .. tostring(os.time()) .. tostring(math.random(10000, 99999)), new_path, min_size)
            if not ok_file then
                MS_UpdateMsg('Не удалось скачать файл обновления. Код: ' .. tostring(status_file))
                os.remove(new_path)
                return
            end

            local installed, install_err = MS_UpdateInstallNow(new_path, target_path, old_path, remote_version)
            if installed then
                MS_UpdateMsg('Обновление установлено на диск. Перезапустите игру или MoonLoader, чтобы загрузить новую версию.')
                return
            end

            MS_UpdateWritePending(new_path, target_path, old_path, remote_version)
            MS_UpdateMsg('Файл скачан, но замена сейчас не удалась: ' .. tostring(install_err) .. '. Updater поставит обновление при следующем запуске.')
        end)
        if not ok_thread then MS_UpdateMsg('Ошибка обновления: ' .. tostring(err_thread)) end
        MS_UPDATE_BUSY = false
    end)
end

function MS_UpdateStartupNotice()
    local path = MS_UpdatePath('update_installed.txt')
    local data = MS_UpdateReadFile(path)
    if data and #data > 0 then
        local kv = MS_UpdateParseKV(data)
        MS_UpdateMsg('Обновление MS Helper установлено. Версия на диске: ' .. tostring(kv.version or '?') .. '.')
        os.remove(path)
    end
end
...
lua_thread.create(function()
    ...
    if type(sampRegisterChatCommand) == 'function' then
        sampRegisterChatCommand('msupdate', MS_UpdateCommand)
    end
    wait(1200)
    MS_UpdateStartupNotice()
end)
