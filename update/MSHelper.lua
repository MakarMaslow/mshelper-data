...
-- =========================================================
MSH_UPDATER_VERSION = MSH_UPDATER_VERSION or '1.0.1'
...
function MSHelper_UpdateRun(arg)
    ...
    lua_thread.create(function()
        ...
        local json_path = wd .. '\\mshelper_update.json'
        local upd_path = thisScript().path .. '.update'
        ...
        downloadUrlToFile(MSH_UPDATER_JSON_URL, json_path, function(id, status)
            if status == 6 then -- DL_STATUS_ENDDOWNLOADDATA
                local raw = MSHelper_UpdateReadFile(json_path)
                pcall(os.remove, json_path)
